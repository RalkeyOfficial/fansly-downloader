...
def display_release_notes(version_string: str, code_contents: str):
    output(LOGLEVEL.UPDATE, f"Successfully updated to version {version_string}\n\n ► Release Notes:{code_contents}")
...
def handle_update(current_version: str, release: dict):
    ...
    output(LOGLEVEL.UPDATE, 'Please be patient, automatic update initialized ...')
    ...
    if not release_download.ok:
        output(LOGLEVEL.ERROR, f"Failed downloading latest build. Release request status code: {release_download.status_code} | Body: \n{release_download.text}")
        return False
